...
#include "RPG_Systems/GameplayAbility/AsyncTask/RPG_AsyncTaskCooldownChanged.h"

URPG_AsyncTaskCooldownChanged* URPG_AsyncTaskCooldownChanged::ListenForCooldownChange(UAbilitySystemComponent * AbilitySystemComponent, FGameplayTagContainer InCooldownTags, bool InUseServerCooldown)
{
	URPG_AsyncTaskCooldownChanged* ListenForCooldownChange = NewObject<URPG_AsyncTaskCooldownChanged>();
	...
	AbilitySystemComponent->OnActiveGameplayEffectAddedDelegateToSelf.AddUObject(ListenForCooldownChange, &URPG_AsyncTaskCooldownChanged::OnActiveGameplayEffectAddedCallback);
	...
	{
		AbilitySystemComponent->RegisterGameplayTagEvent(CooldownTag, EGameplayTagEventType::NewOrRemoved).AddUObject(ListenForCooldownChange, &URPG_AsyncTaskCooldownChanged::CooldownTagChanged);
	}

	return ListenForCooldownChange;
}

void URPG_AsyncTaskCooldownChanged::EndTask()
{
	if (IsValid(ASC))
	{
...
void URPG_AsyncTaskCooldownChanged::CooldownTagChanged(const FGameplayTag CooldownTag, int32 NewCount)
{
	if (NewCount == 0)
	{
...
bool URPG_AsyncTaskCooldownChanged::GetCooldownRemainingForTag(FGameplayTagContainer InCooldownTags, float & TimeRemaining, float & CooldownDuration)
{
