...
{
	if (!UKismetSystemLibrary::IsDedicatedServer(this) && OwnerCharacter)
	{

		FHitResult hit;
		FVector Start = OwnerCharacter->GetMesh()->GetSocketLocation(Socket);
		Start.Z += 50;
		FVector End = OwnerCharacter->GetMesh()->GetSocketLocation(Socket);
		End.Z -= 50;
